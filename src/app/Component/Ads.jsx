...
const Ads = () => {
    ...
    useEffect(() => {
        const fetchAds = async () => {
            try {
                const response = await fetch('/api/post'); // Replace with your API endpoint
                if (!response.ok) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
            {ads.map((ad) => (
                ...
                >
                    ...
                    {ad.image && (
                        <img
                        src={`/api/post/download?fileId=${ad.image._id}`}
                            alt="Ad"
                    ...
                    {ad.video && (
                        <video
                            ...
                            loop
                        >
                            <source src={`/api/post/download?fileId=${ad.video._id}`} type="video/mp4" />
